    ...
    */
    ...
    for (int i = beginx; i < endx; ++i) {
        ...
        if (D_rank == MPI_PROC_NULL ){
           v[IDX_mini(i,1)]    = 2.0 * dy2i * (s[IDX((B_x-1)+i,0)]    - s[IDX((B_x-1)+i,1)]);
        //    cout << (B_x-1)+i << "  "; 
        }   
        // top       
        if (U_rank == MPI_PROC_NULL ){
             v[IDX_mini(i,endy)] = 2.0 * dy2i * (s[IDX((B_x-1)+i,Ny-1)] - s[IDX((B_x-1)+i,Ny-2)])
                       - 2.0 * dyi*U;
            // cout << "("<<i << " " << endy << ")  "; 
        }
    ...
    for (int j = beginx; j < endy; ++j) {
        ...
        if (L_rank == MPI_PROC_NULL){
            v[IDX_mini(1,j)]    = 2.0 * dx2i * (s[IDX(0,j+(B_y-1))]    - s[IDX(1,j+(B_y-1))]);
        }      
        // right
        if (R_rank == MPI_PROC_NULL){
            v[IDX_mini(endx,j)] = 2.0 * dx2i * (s[IDX(Nx-1,j+(B_y-1))] - s[IDX(Nx-2,j+(B_y-1))]);
        }    
    }
 
    MPI_Barrier(Mycart_grid);
    ...
    for (int i = beginx; i < endx; ++i) {
        for (int j = beginy; j < endy; ++j) {
            ...
            vgather[IDX(i+(B_x-1),j+(B_y-1))] =  vnew[IDX_mini(i,j)]; 
            // MPI_Igather(&vnew[IDX_mini(i,j)],1, MPI_DOUBLE, &vsum[IDX(i+(B_x-1),j+(B_y-1))], (endx)*(endy), MPI_DOUBLE,0,MPI_COMM_WORLD,&sendrequest);
    ...
    MPI_Reduce(vgather,&vsum[0], Nx*Ny, MPI_DOUBLE, MPI_SUM, 0,MPI_COMM_WORLD);
