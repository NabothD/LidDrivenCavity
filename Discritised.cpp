...
void LidDrivenCavity::Initialise()
{
    ...
    v   = new double[(E_x-B_x)*(E_y-B_y)]();
    vnew = new double[(E_x-B_x)*(E_y-B_y)]();
    vgather = new double[Npts]();
    sgather = new double[Npts]();
    s   = new double[(E_x-B_x)*(E_y-B_y)]();
    tmp = new double[Npts]();
    ...
    */
    ...
    if(Myworldsize > 0){
        ...
        if (D_rank != MPI_PROC_NULL && Myrank){
            ...
            MPI_Isend(&s[IDX_mini(i, 1)], Ex, MPI_DOUBLE, D_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
            ...
            MPI_Irecv(&v[IDX_mini(i,Ey)],Ex, MPI_DOUBLE, U_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
             MPI_Irecv(&s[IDX_mini(i,Ey)],Ex, MPI_DOUBLE, U_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
        ...
        if (U_rank != MPI_PROC_NULL&& Myrank){
            MPI_Isend(&v[IDX)*(Eyini(i, Ey)], 1, MPI_DOUBLE, U_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
            MPI_Isend(&s[IDX_mini(i, Ey)],1, MPI_DOUBLE, U_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
            MPI_Irecv(&v[IDX_mini(i,0)], 1,MPI_DOUBLE, D_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
            MPI_Irecv(&s[IDX_mini(i,0)], 1,MPI_DOUBLE, D_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
            // MPI_Sendrecv(&v[IDX(i, E_y)], 1, MPI_DOUBLE, U_rank, tag_send, &v[IDX(i,B_y-1)], 1,
            //         MPI_DOUBLE, Myrank, tag_recv, Mycart_grid, MPI_STATUS_IGNORE ); // Sending stuff up
            // cout << v[IDX(i, E_y)] << endl;     
        }
        ...
        for (int j = 1; j < Ey; ++j) {
            if (L_rank != MPI_PROC_NULL && Myrank){
                ...
                MPI_Isend(&v[IDX_mini(1, j)], 1, MPI_DOUBLE, L_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
                MPI_Isend(&s[IDX_mini(1, j)], 1, MPI_DOUBLE, L_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
                MPI_Irecv(&v[IDX_mini(Ex,j)], 1, MPI_DOUBLE, R_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
                MPI_Irecv(&s[IDX_mini(Ex,j)], 1, MPI_DOUBLE, R_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);

                // MPI_Sendrecv(&v[IDX(B_x, j)], 1, MPI_DOUBLE, L_rank, tag_send, &v[IDX(E_x,j)], 1,
                //             MPI_DOUBLE, Myrank, tag_recv, Mycart_grid, MPI_STATUS_IGNORE ); // Sending stuff to the left wall  
            }
    
            if (R_rank != MPI_PROC_NULL && Myrank){
                MPI_Isend(&v[IDX_mini(Ex, j)], 1, MPI_DOUBLE, R_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
                MPI_Isend(&s[IDX_mini(Ex, j)], 1, MPI_DOUBLE, R_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);


                MPI_Irecv( &v[IDX_mini(0,j)], 1, MPI_DOUBLE, L_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
                MPI_Irecv( &s[IDX_mini(0,j)], 1, MPI_DOUBLE, L_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);


                // MPI_Sendrecv(&v[IDX(E_x, j)], 1, MPI_DOUBLE, R_rank, tag_send, &v[IDX(B_x-1,j)], 1,
                //         MPI_DOUBLE, Myrank, tag_recv, Mycart_grid, MPI_STATUS_IGNORE ); // Sending stuff to the right
            }       
        }
    ...
    for (int i = 1; i < Ex; ++i) {
        for (int j = 1; j < Ey; ++j) {
            ...
            MPI_Igather(&s[IDX_mini(i,j)],1, MPI_DOUBLE, &sgather[IDX(i+(B_x-1),j+(B_y-1))], (Ex)*(Ey), MPI_DOUBLE,0,Mycart_grid,&sendrequest[requestcount++]);
            // // MPI_Gather(&vnew[IDX(i,j)],(E_x-B_x)*(E_y-B_y), MPI_DOUBLE, &vgather[IDX(i,j)], (E_x-B_x)*(E_y-B_y), MPI_DOUBLE,0,Mycart_grid);
            
        }

    }

    // MPI_Gather(&vnew,E_x*E_y, MPI_DOUBLE, &vgather, E_x*E_y, MPI_DOUBLE,0,Mycart_grid);
    // MPI_Allreduce(&vnew, &vgather, Nx * Ny, MPI_DOUBLE, MPI_SUM, Mycart_grid);
    /**
