...
void LidDrivenCavity::Advance()
{
    ...
    */
    ...
    MPI_Request recvrequest[Nx*Ny];
    MPI_Request sendrequest[Nx*Ny];
    int requestcount = 0; 
    // Interior boundary conditions
    for (int i = B_x; i < E_x; ++i) {
        if (D_rank != MPI_PROC_NULL && Myrank){

            MPI_Isend(&v[IDX(i, B_y)], 1, MPI_DOUBLE, D_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
            MPI_Irecv(&v[IDX(i,E_y)], 1, MPI_DOUBLE, U_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
            cout << "I just Recieved this from the rank above: " << v[IDX(i,E_y)] << endl; 

            // MPI_Sendrecv(&v[IDX(i, B_y)], 1, MPI_DOUBLE, D_rank, tag_send, &v[IDX(i,E_y)], 1,
            //         MPI_DOUBLE, Myrank, tag_recv, Mycart_grid, MPI_STATUS_IGNORE ); // Sending stuff down 
        }
        
        if (U_rank != MPI_PROC_NULL&& Myrank){
            MPI_Isend(&v[IDX(i, E_y)], 1, MPI_DOUBLE, U_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
            MPI_Irecv(&v[IDX(i,B_y-1)], 1,MPI_DOUBLE, D_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
    ...
    for (int j = B_y; j < E_y; ++j) {
        if (L_rank != MPI_PROC_NULL && Myrank){
            ...
            MPI_Isend(&v[IDX(B_x, j)], 1, MPI_DOUBLE, L_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);
            MPI_Irecv(&v[IDX(E_x,j)], 1, MPI_DOUBLE, R_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
        ...
        if (R_rank != MPI_PROC_NULL && Myrank){
            MPI_Isend(&v[IDX(E_x, j)], 1, MPI_DOUBLE, R_rank, tag_send,Mycart_grid, &sendrequest[requestcount++]);

            MPI_Irecv( &v[IDX(B_x-1,j)], 1, MPI_DOUBLE, L_rank, tag_recv, Mycart_grid, &recvrequest[requestcount++]);
    ...
    for (int i = B_x; i < E_x; ++i) {
        for (int j = B_y; j < E_y; ++j) {
            ...
            // MPI_Alltoall(&vnew[IDX(i,j)],1, MPI_DOUBLE, &vgather[IDX(i,j)],1,MPI_DOUBLE,MPI_COMM_WORLD);
